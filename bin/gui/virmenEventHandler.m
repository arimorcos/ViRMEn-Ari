...
switch type
    ...
    case 'shapeClick'
        ...
        switch str
            ...
            case {'open','color'}
                ...
                if strcmp(class(handles.exper.worlds{wNum}.objects{oNum}.texture.shapes{inp{1}}),'shapeColor')
    ...
    case 'addShape'
        ...
        if length(handles.exper.worlds{wNum}.objects{oNum}.texture.shapes)==2 ...
                & strcmp(class(handles.exper.worlds{wNum}.objects{oNum}.texture.shapes{2}),'shapeImage') %#ok<AND2>
        ...
        try
            ...
            set(get(gca,'title'),'string',obj.helpString);
            obj = getPoints(obj);
            set(get(gca,'title'),'string',obj.helpString);
    ...
    case 'textureSketchClick'
        switch get(guifig,'selectiontype')
            case 'normal'
                set(currax,'units','pixels');
                set(get(currax,'parent'),'units','pixels');
                set(get(get(currax,'parent'),'parent'),'units','pixels');
                ...
                pos = get(currax,'position');
                pos2 = get(get(currax,'parent'),'position');
                pos(1:2) = pos(1:2)+pos2(1:2);
                set(get(get(currax,'parent'),'parent'),'units','normalized');
                set(get(currax,'parent'),'units','normalized');
                set(currax,'units','normalized');
                xl = get(currax,'xlim');
                yl = get(currax,'ylim');
    ...
    case 'deleteObject'
        if oNum == 0
            errordlg('Cannot delete start location.','Error');
            set(guifig,'Pointer','arrow'); return
        end
    ...
    case 'renameObject'
        if oNum == 0
            errordlg('Cannot rename start location.','Error');
            set(guifig,'Pointer','arrow'); return
        end
    ...
    case 'addObjectLocation'
        if oNum == 0
            errordlg('Use properties to motify start location.','Error');
            set(guifig,'Pointer','arrow'); return
        end
