...
if ismac && ~exist('/usr/local/lib/libglfw.3.dylib','file')
    try
        copyfile([path '/../engine/libglfw.3.dylib'], '/usr/local/lib/libglfw.3.dylib');
    catch %#ok<CTCH>
...
for ndx = 1:length(mt)
    copyfile([path filesep '..' filesep 'gui' filesep 'builtinShapes' filesep mt(ndx).name], ...
...
for ndx = 1:length(mt)
    copyfile([path filesep '..' filesep 'gui' filesep 'builtinObjects' filesep mt(ndx).name], ...
...
for ndx = 1:length(mt)
    copyfile([path filesep '..' filesep 'gui' filesep 'builtinTransformations' filesep mt(ndx).name], ...
...
for ndx = 1:length(mt)
    copyfile([path filesep '..' filesep 'gui' filesep 'builtinMovements' filesep mt(ndx).name], ...
