    ...
    switch row{colm.Type}
        ...
        case 'dropdown'
            ...
            for m = 1:length(lst)
                ...
                set(h{m},'ActionPerformedCallback',['virmenEventHandler(''' lst(m).callback ''',''' lst(m).callbackArgument ''');']);
            end
            drawnow
            ...
            for mndx = 1:length(h)
                uimenu(m,'Label',lst(mndx).name,'callback',get(h{mndx},'ActionPerformedCallback'),'userdata','n/a');
            end
        case 'toggle'
            if ~strcmp(row{colm.Icon},'n/a')
...
f = findall(guifig,'type','uimenu','label','NA');
delete(f);

for ndx = 1:length(handles.shortcuts)
    f = findall(guifig,'callback',['virmenEventHandler(''' handles.shortcuts(ndx).callback ''',''' handles.shortcuts(ndx).input ''');'],'type','uimenu');
    g = findall(guifig,'callback',['virmenEventHandler(''' handles.shortcuts(ndx).callback ''',{''switch'',''' handles.shortcuts(ndx).input '''});'],'type','uimenu');
    f = union(f,g);
    
    if strcmp(handles.shortcuts(ndx).modifier,'control') && length(handles.shortcuts(ndx).key)==1
        set(f,'accelerator',handles.shortcuts(ndx).key);
    end
end
