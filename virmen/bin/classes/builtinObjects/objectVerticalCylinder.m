        ...
        function [x y z] = coords2D(obj)
            ...
            for zs = zlist
                for ndx = 1:size(obj.locations,1)
                    x = [x; xs+obj.locations(ndx,1); NaN]; %#ok<AGROW>
                    y = [y; ys+obj.locations(ndx,2); NaN]; %#ok<AGROW>
                    z = [z; zs*ones(size(xs)); NaN]; %#ok<AGROW>
                    for th = thlist
                        x = [x; obj.radius*cosd([th; th])+obj.locations(ndx,1); NaN]; %#ok<AGROW>
                        y = [y; obj.radius*sind([th; th])+obj.locations(ndx,2); NaN]; %#ok<AGROW>
