...
%specify probabilities
              % HARD           MEDIUM       EASY
vr.mazeProbs = [0.30/1         0.40/1       0.45/1;...  %a
                0.30/2         0.30/2       0.35/2;...  %b
                0.30/2         0.30/2       0.35/2;...  %c
                0.25/2         0.20/2       0.15/2;...  %d
                0.25/2         0.20/2       0.15/2;...  %f
                0.15/2         0.10/2       0.05/2;...  %g
                0.15/2         0.10/2       0.05/2];    %h

%single tower crutch
vr.alternateCrutch = false; %should crutch trials alternate w/ non-crutch? If false, use probCrutch to determine
vr.adaptiveCrutch = false; %should crutch trials be set to %incorrect
...
if vr.cuePos == 1
    ...
    vr.numLeft = length(vr.leftDotLoc);
else
