function code = DMSDiscreetIntGraded2Towers
% DMSDiscreetIntGraded2Towers   Code for the ViRMEn experiment DMSDiscreetIntGraded2Towers.
%   code = DMSDiscreetIntGraded2Towers   Returns handles to the functions that ViRMEn
...
vr.condProbs(3,:) = [3/7 3/7 1/7 0]; %condition probabilities for the up to 1-3 condition. Must add up to 1
vr.condProbs(4,:) = [2/5 2/5 1/5 0]; %condition probabilities for the up to 1-3 condition. Must add up to 1
vr.condProbs(5,:) = [0.4 0.3 0.3 0]; %condition probabilities for the up to 1-3 condition. Must add up to 1
vr.condProbs(6,:) = [0.35 0.3 0.3 0.05]; %condition probabilities for the all condition. Must add up to 1
vr.intGraded = 1; %0 - only 0-4, 1 - up to 1-3, 2- all

vr.greyFac = 15/63; %goes from 0 to 1 to signify the amount of maze which is grey
vr.numRewPer = 1;
vr.numSeg = 6; %set numHorizText to 20*numSeg
...
vr.crutchIdentity = 'Continuity'; %identity of crutch trial
...
behindDelay = vr.behindDelay(1):vr.behindDelay(2);
...
vr.blackLeftCrutch = [behindDelay beginBlack vr.whiteRight backBlack vr.whiteRightTowers];
vr.blackRightCrutch = [behindDelay beginBlack vr.whiteLeft backBlack vr.whiteLeftTowers];
vr.whiteLeftCrutch = [behindDelay beginWhite vr.whiteLeft backWhite vr.whiteLeftTowers];
vr.whiteRightCrutch = [behindDelay beginWhite vr.whiteRight backWhite vr.whiteRightTowers];
...
if vr.inITI == 0 && abs(vr.position(1)) > eval(vr.exper.variables.armLength)/vr.armFac &&...
    ...
    vr.trialResults(5,end) = vr.crutchTrial;
else
    vr.isReward = 0;
end
...
if vr.inITI == 1
    ...
    if vr.cellWrite
        [dataStruct] = createSaveStruct(vr.mouseNum,vr.experimenter,...
            ...
            'numWhite',vr.numWhite,'intGraded',vr.intGraded,'crutchIdentity',vr.crutchIdentity,...
            'crutchTrial',vr.crutchTrial); %#ok<NASGU>
    ...
    if toc(vr.trialsStart) < vr.breakThreshTime || ~vr.breaks
        ...
        if vr.itiTime > vr.itiDur
            ...
            elseif vr.adaptiveCrutch
                nonCrutchSub = vr.trialResults(:,vr.trialResults(5,:)==0);
