function code = AMDMSMaze2WallsAdaptiveNoTower
% AMDMSMaze2WallsAdaptiveNoTower   Code for the ViRMEn experiment AMDMSMaze2WallsAdaptiveNoTower.
%   code = AMDMSMaze2WallsAdaptiveNoTower   Returns handles to the functions that ViRMEn
...
if vr.inITI == 0 && abs(vr.position(1)) > eval(vr.exper.variables.armLength)/2 && vr.position(2) > eval(vr.exper.variables.MazeLengthAhead)
    ...
    if vr.isReward == 1 && (vr.cuePos == 3 || vr.cuePos == 4)
        vr.numBlackTurn = vr.numBlackTurn + 1;
    elseif vr.isReward == 0 && (vr.cuePos == 1 || vr.cuePos == 2)
        vr.numBlackTurn = vr.numBlackTurn + 1;
    end
    
    if vr.isReward == 1 && (vr.cuePos == 1 || vr.cuePos == 3)
        vr.numLeftTurn = vr.numLeftTurn + 1;
    elseif vr.isReward == 0 && (vr.cuePos == 2 || vr.cuePos == 4)
        vr.numLeftTurn = vr.numLeftTurn + 1;
    end
