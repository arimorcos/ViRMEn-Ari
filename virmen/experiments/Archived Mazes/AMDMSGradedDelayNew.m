function code = AMDMSGradedDelay
% AMDMSGradedDelay   Code for the ViRMEn experiment AMDMSGradedDelay.
%   code = AMDMSGradedDelay   Returns handles to the functions that ViRMEn
...
%Set up data array allocation
vr.index = 1;
vr.allocationSize = 100000;
vr.data = nan(10,vr.allocationSize);

%Get initial delay
vr.percGraded = str2double(vr.exper.variables.initPercGraded);
vr.percEndGraded = (100-vr.percGraded)/100;
vr.numTrialsDelayLevel = 0;
vr.numRewardsDelayLevel = 0;
vr.numLeftTurnsDelay = 0;
vr.trialResults = [];
...
vr.text(5).size = .03;
vr.text(5).color = [0 .5 0];

vr.text(6).string = '0';
vr.text(6).position = [1 .3];
vr.text(6).size = .03;
vr.text(6).color = [.5 .5 0];

vr.text(7).string = '0';
vr.text(7).position = [1 .2];
vr.text(7).size = .03;
vr.text(7).color = [.5 0 .5];

vr.text(8).string = '0';
vr.text(8).position = [1 .1];
vr.text(8).size = .03;
vr.text(8).color = [0 0 .5];
...
if vr.inITI == 0 && abs(vr.position(1)) > eval(vr.exper.variables.armLength)/2 && vr.position(2) > eval(vr.exper.variables.MazeLengthAhead)
    if vr.position(1) < 0 && (vr.cuePos == 1 || vr.cuePos == 3)
        ...
        vr.numRewardsDelayLevel = vr.numRewardsDelayLevel + 1;
        vr.trialResults(size(vr.trialResults)+1) = 1;
    elseif  vr.position(1) > 0 && (vr.cuePos == 2 || vr.cuePos == 4)
        ...
        vr.isReward = 0;
        vr.itiDur = 4;
        vr.trialResults(size(vr.trialResults)+1) = 0;
    end
    ...
    vr.numTrialsDelayLevel = vr.numTrialsDelayLevel + 1;
    
    if ((vr.cuePos == 1 || vr.cuePos == 3) && vr.isReward ~= 0) || ((vr.cuePos == 2 || vr.cuePos == 4) && vr.isReward == 0)
        vr.numLeftTurnsDelay = vr.numLeftTurnsDelay + 1;
    end
else
    vr.isReward = 0;
end
...
if vr.inITI == 1
    ...
    if vr.itiTime > vr.itiDur
        ...
        %update percGraded
        vr.percCorrectDelay = vr.numRewardsDelayLevel/vr.numTrialsDelayLevel;
        if vr.numTrialsDelayLevel >= vr.trialThresh && vr.percGraded < 50
            vr.percCorrectLast = sum(vr.trialResults(end-vr.trialThresh+1:end))/vr.trialThresh;
            if vr.percCorrectLast >= vr.percThreshAdv 
                vr.percGraded = vr.percGraded + 5;
                vr.percEndGraded = (100-vr.percGraded)/100;
                vr.numRewardsDelayLevel = 0;
                vr.numTrialsDelayLevel = 0;
                vr.numLeftTurnsDelay = 0;
            end
        elseif vr.numTrialsDelayLevel >= vr.trialThresh && vr.percGraded > 10
            vr.percCorrectLast = sum(vr.trialResults(end-vr.trialThresh+1:end))/vr.trialThresh;
            if vr.percCorrectLast <= vr.percThreshReg 
                vr.percGraded = vr.percGraded - 5;
                vr.percEndGraded = (100-vr.percGraded)/100;
                vr.numRewardsDelayLevel = 0;
                vr.numTrialsDelayLevel = 0;
        ...
        vr.percLeftDelay = vr.numLeftTurnsDelay/vr.numTrialsDelayLevel;
        randLeft = rand;
        randColor = rand;
        if randLeft >= vr.percLeftDelay
            if randColor >= 0.5
                vr.cuePos = 1;
            else
                vr.cuePos = 3;
...
vr.percCorrectDelay = 100*vr.numRewardsDelayLevel/vr.numTrialsDelayLevel;
if isnan(vr.percCorrect)
    vr.percCorrect = 0;
end
if isnan(vr.percCorrectDelay)
    vr.percCorrectDelay = 0;
end
...
vr.text(5).string = ['PERCGRADED ',num2str(vr.percGraded)];
vr.text(6).string = ['DELTRIALS ', num2str(vr.numTrialsDelayLevel)];
vr.text(7).string = ['DELREWARDS ',num2str(vr.numRewardsDelayLevel)];
vr.text(8).string = ['DELAYPERC ',num2str(round(vr.percCorrectDelay))];
...
if vr.shouldUDP
    ...
    fwrite(vr.udp,[double(datestr(now - vr.startTime,'HH:MM:SS')) vr.numTrials vr.numRewards...
        100*vr.numRewards/vr.numTrials vr.percGraded vr.numTrialsDelayLevel vr.numRewardsDelayLevel...
        100*vr.numRewardsDelayLevel/vr.numTrialsDelayLevel vr.turnOffUDP]);
end
...
if size(vr.data,2) >= vr.allocationSize
    vr.data(:,size(vr.data,2):size(vr.data,2)+vr.allocationSize-1) = nan(10,vr.allocationSize);
end

% vr.data(:,vr.index) = [rem(now,1) vr.position([1:2,4]) vr.velocity(1:2) vr.cuePos vr.isReward vr.inITI vr.percGraded]';
...
fclose all;
vr.data = vr.data(:,any(~isnan(vr.data),1));
save(vr.pathMat,'-struct','vr','data','-append');
fclose all;
if vr.shouldUDP
    delete(vr.udp);
end
