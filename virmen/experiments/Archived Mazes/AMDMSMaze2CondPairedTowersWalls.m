function code = AMDMSMaze2CondPairedTowersWalls
% AMDMSMaze2CondPairedTowersWalls   Code for the ViRMEn experiment AMDMSMaze2CondPairedTowersWalls.
%   code = AMDMSMaze2CondPairedTowersWalls   Returns handles to the functions that ViRMEn
...
if vr.inITI == 0 && abs(vr.position(1)) > eval(vr.exper.variables.armLength)/2 && vr.position(2) > eval(vr.exper.variables.MazeLengthAhead)
    if vr.position(1) < 0 && (vr.currentWorld == 1 || vr.currentWorld == 2)
        if ~vr.debugMode
        ...
        vr.itiDur = 2;
    elseif  vr.position(1) > 0 && (vr.currentWorld == 3 || vr.currentWorld == 4)
        if ~vr.debugMode    
            putdata(vr.ao,[[5;zeros(5,1)],zeros(6,1),zeros(6,1),zeros(6,1)]);
            start(vr. ao);
            stop(vr.ao);
    ...
    if vr.isReward ~= 0 && (vr.currentWorld == 1 || vr.currentWorld == 2)
        vr.numBlackTurn = vr.numBlackTurn + 1;
    elseif vr.isReward == 0 && (vr.currentWorld == 3 || vr.currentWorld == 4)
        vr.numBlackTurn = vr.numBlackTurn + 1;
    end
    ...
    if vr.currentWorld == 1 || vr.currentWorld == 3
        vr.numTrialsTower = vr.numTrialsTower + 1;
    elseif vr.currentWorld == 2 || vr.currentWorld == 4
