function code = DMSIntegrationGraded
% DMSIntegrationGraded   Code for the ViRMEn experiment DMSIntegrationGraded.
%   code = DMSIntegrationGraded   Returns handles to the functions that ViRMEn
...
vr.intGraded = str2double(vr.exper.variables.initIntGraded);
switch vr.intGraded
    case 8
        vr.dotMax = 0;
        vr.dotMin = 8;
...
if vr.cuePos <= 2
    numWhite = randi([0 vr.dotMax],1);
else
    numWhite = randi([vr.dotMin 8],1);
end
...
if vr.inITI == 0 && abs(vr.position(1)) > eval(vr.exper.variables.armLength)/2 && vr.position(2) > eval(vr.exper.variables.MazeLengthAhead)
    if vr.position(1) < 0 && (vr.cuePos == 1 || vr.cuePos == 3)
        ...
        vr.numRewGraded = vr.numRewGraded + 1;
    ...
    elseif  vr.position(1) > 0 && (vr.cuePos == 2 || vr.cuePos == 4)
        ...
        vr.numRewards = vr.numRewards + 1;
        vr.numRewGraded = vr.numRewGraded + 1;
        vr.trialResults(vsize(vr.trialResults)+1) = 1;
    else
        vr.isReward = 0;
        vr.itiDur = 4;
    ...
    vr.numTrialsGraded = vr.numTrialsGraded + 1;
...
if vr.inITI == 1
    ...
    if vr.itiTime > vr.itiDur
        ...
        %check for change in integration graded 
        vr.percCorrGraded = vr.numRewGraded/vr.numTrialsGraded;
        if vr.numTrialsGraded >= vr.trialThresh && vr.percCorrGraded >= vr.advThresh && vr.intGraded > 5
            vr.intGraded = vr.intGraded - 1;
            vr.numTrialsGraded = 0;
            vr.numRewGraded = 0;
        elseif vr.numTrialsGraded >= vr.trialThresh && vr.percCorrGraded <= vr.regThresh && vr.intGraded < 8
            vr.intGraded = vr.intGraded + 1;
            vr.numTrialsGraded = 0;
            vr.numRewGraded = 0;
        end
        ...
        switch vr.intGraded
            ...
            case 5
                ...
                vr.dotMin = 4;
            otherwise 
                error('Cannot process intGraded');
        end
        if vr.cuePos <= 2
            numWhite = randi([0 vr.dotMax],1);
        else
...
vr.percCorrectGraded = 100*vr.numRewGraded/vr.numTrialsGraded;
if isnan(vr.percCorrectGraded)
    vr.percCorrectGraded = 0;
end
...
vr.text(5).string = ['INTCOND ',num2str(vr.intGraded)];
vr.text(6).string = ['INTTRIALS ', num2str(vr.numTrialsGraded)];
vr.text(7).string = ['INTREWARDS ',num2str(vr.numRewGraded)];
vr.text(8).string = ['INTPERC ',num2str(round(vr.percCorrectGraded))];


fwrite(vr.fid,[rem(now,1) vr.position([1:2,4]) vr.velocity(1:2) vr.cuePos vr.isReward vr.inITI vr.intGraded],'float');
