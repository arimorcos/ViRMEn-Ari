function code = AMDMSMaze2WallsEnd
% AMDMSMaze2WallsEnd   Code for the ViRMEn experiment AMDMSMaze2WallsEnd.
%   code = AMDMSMaze2WallsEnd   Returns handles to the functions that ViRMEn
...
if vr.inITI == 0 && abs(vr.position(1)) > eval(vr.exper.variables.armLength)/2 && vr.position(2) > eval(vr.exper.variables.MazeLengthAhead)
    if vr.position(1) < 0 && vr.cuePos == 2
        vr.isReward = 1;
        if ~vr.debugMode
    ...
    elseif  vr.position(1) > 0 && vr.cuePos == 1
        vr.isReward = 1;
        if ~vr.debugMode
            putdata(vr.ao,[[5;zeros(5,1)],zeros(6,1),zeros(6,1),zeros(6,1)]);
            start(vr. ao);
