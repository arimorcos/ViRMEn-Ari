function code = ContinuityAlcoveGraded
% ContinuityAlcoveGraded   Code for the ViRMEn experiment ContinuityAlcoveGraded.
%   code = ContinuityAlcoveGraded   Returns handles to the functions that ViRMEn
...
vr.scaleFac = 0.4; %0 is full alcove, 1 is continuity
vr.trialThresh = 40;
vr.percThreshAdv = .75;
...
vr.lengthThresh = 0.5; %scale factor threshold for length to move
...
vr.alcWalls = vr.worlds{1}.objects.triangles(vr.worlds{1}.objects.indices.alcWalls,:);
...
vr.alcWallsT = vr.alcWalls(1):vr.alcWalls(2);
...
if vr.inITI == 1
    ...
    if vr.itiTime > vr.itiDur
        ...
        %update lengthFac
        if size(vr.trialResults,2) > vr.numTrialsReg + 2
            if sum(vr.trialResults(1,end-1:end)) == 2 && vr.scaleFac <= vr.lengthThresh && vr.lengthFac <= (vr.maxLenFac - vr.forRateLen)
                vr.lengthFac = vr.lengthFac + vr.forRateLen;
            elseif sum(vr.trialResults(1,end-vr.numTrialsReg+1:end)) == 0 && vr.scaleFac > 1 + vr.revRateLen
