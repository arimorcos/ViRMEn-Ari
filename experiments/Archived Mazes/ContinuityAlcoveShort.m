function code = ContinuityAlcoveShort
% ContinuityAlcove   Code for the ViRMEn experiment ContinuityAlcove.
%   code = ContinuityAlcove   Returns handles to the functions that ViRMEn
...
lastLeft = vr.LeftWallDelay(1)+floor((vr.LeftWallDelay(2)-vr.LeftWallDelay(1))*(vr.segLength/500));
lastRight = vr.RightWallDelay(1)+floor((vr.RightWallDelay(2)-vr.RightWallDelay(1))*(vr.segLength/500));
for i = 1:8
    vr.worlds{1}.surface.visible(lastLeft:lastLeft+floor((vr.LeftWallDelay(2)-vr.LeftWallDelay(1))*(vr.alcLength/500))) = 1;
    vr.worlds{1}.surface.visible(lastRight:lastRight+floor((vr.RightWallDelay(2)-vr.RightWallDelay(1))*(vr.alcLength/500))) = 1;
    lastLeft = lastLeft+1+floor((vr.LeftWallDelay(2)-vr.LeftWallDelay(1))*(vr.segLength + vr.alcLength)/500);
    lastRight = lastRight+1+floor((vr.RightWallDelay(2)-vr.RightWallDelay(1))*(vr.segLength + vr.alcLength)/500);
end
