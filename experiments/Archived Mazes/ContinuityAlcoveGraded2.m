function code = ContinuityAlcoveGraded2
% ContinuityAlcoveGraded2   Code for the ViRMEn experiment ContinuityAlcoveGraded2.
%   code = ContinuityAlcoveGraded2   Returns handles to the functions that ViRMEn
...
%set visibility of alcove segments (grey)
vr.scaleFac = 0;

vr.segTri = round((vr.LeftWallDelay(2) - vr.LeftWallDelay(1))/16);
for i = 2:2:16
    vr.worlds{1}.surface.visible(vr.LeftWallDelay(1)+vr.scaleFac*vr.segTri+ceil((vr.LeftWallDelay(2)-vr.LeftWallDelay(1))*(i-1)/16):...
            vr.LeftWallDelay(1)+floor((vr.LeftWallDelay(2)-vr.LeftWallDelay(1))*i/(16))) = 1;
    vr.worlds{1}.surface.visible(vr.RightWallDelay(1)+vr.scaleFac*vr.segTri+ceil((vr.RightWallDelay(2)-vr.RightWallDelay(1))*(i-1)/16):...
        vr.RightWallDelay(1)+floor((vr.RightWallDelay(2)-vr.RightWallDelay(1))*i/(16))) = 1;
end

%get alcove wall vertices (only want every other from 1 to 15)
for i = 1:2:15
    eval(['vr.alcWalls(',num2str(i),',:) = vr.worlds{1}.objects.vertices(vr.worlds{1}.objects.indices.alcWalls',num2str(i),',:)']);
end

%get delay wall vertices
vr.LeftWallDelayVert = vr.worlds{1}.objects.vertices(vr.worlds{1}.objects.indices.LeftWallDelay,:);
vr.RightWallDelayVert = vr.worlds{1}.objects.vertices(vr.worlds{1}.objects.indices.RightWallDelay,:);

vr.segVert = str2double(vr.exper.variables.segLength);

%move alcove walls

for i = 1:2:15
    vr.alcWallsRoot(i,:) = vr.worlds{1}.surface.vertices(2,vr.alcWalls(i,1):vr.alcWalls(i,2));
    vr.worlds{1}.surface.vertices(2,vr.alcWalls(i,1):vr.alcWalls(i,2)) = vr.alcWallsRoot(i,:) + vr.scaleFac*vr.segVert;
end

% %store delay walls root
% vr.leftDelayRoot = vr.worlds{1}.surface.vertices(1,vr.LeftWallDelay(1):vr.LeftWallDelay(2));
% vr.rightDelayRoot = vr.worlds{1}.surface.vertices(1,vr.RightWallDelay(1):vr.RightWallDelay(2));
% 
% vr.alcWalls
% 
% %move delay walls
% vr.worlds{1}.surface.vertices(1,vr.LeftWallDelay(1):vr.LeftWallDelay(2)) = vr.worlds{1}.surface.vertices(1,vr.LeftWallDelay(1):vr.LeftWallDelay(2)) +
...
if vr.inITI == 1
    ...
    if vr.itiTime > vr.itiDur
        ...
        vr.scaleFac = vr.scaleFac + 0.25;

        if vr.scaleFac > 0.75
            vr.scaleFac = 0;
            for i = 1:2:15
                vr.worlds{1}.surface.vertices(2,vr.alcWalls(i,1):vr.alcWalls(i,2)) = vr.alcWallsRoot(i,:);
            end
        end
