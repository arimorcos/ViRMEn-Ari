function code = DMSDiscreetIntGradedShort
% DMSDiscreetIntGradedShort   Code for the ViRMEn experiment DMSDiscreetIntGradedShort.
%   code = DMSDiscreetIntGradedShort   Returns handles to the functions that ViRMEn
...
vr.intGraded = 0; %0 - only 0-8, 1 - up to 1-7, 2- up to 2-6, 3-all

vr.greyFac = 2/7; %goes from 0 to 1 to signify the amount of maze which is grey
vr.numRewPer = 1;
vr.numSeg = 8;
vr.greyRat = 0.2; %goes from 0 to 1 to signify percentage of each segment which is gray. Must be related to number of textures greyRat*((intLength/mazeLength)*numText)/numSeg must be an integer
vr.flashOffset = 0; %offset in virmen units of when flash occurs
...
vr.backDelayWalls = vr.worlds{1}.objects.triangles(vr.worlds{1}.objects.indices.backDelayWalls,:);
...
backDelayWalls = vr.backDelayWalls(1):vr.backDelayWalls(2);

vr.blackLeftOn = [vr.whiteRight dirTower backBlack TTopMiddle backDelayWalls];
vr.blackLeftOff = [vr.whiteLeft delayWalls];

vr.blackRightOn = [vr.whiteLeft dirTower backBlack TTopMiddle backDelayWalls];
vr.blackRightOff = [vr.whiteRight delayWalls];

vr.whiteLeftOn = [vr.whiteLeft dirTower backWhite TTopMiddle backDelayWalls];
vr.whiteLeftOff = [vr.whiteRight delayWalls];

vr.whiteRightOn = [vr.whiteRight dirTower backWhite TTopMiddle backDelayWalls];
...
if vr.inITI == 1
    ...
    if vr.cellWrite
        [dataStruct] = createSaveStruct(vr.mouseNum,vr.experimenter,...
            ...
            'numWhite',vr.numWhite,'intGraded',vr.intGraded,'breaks',vr.breaks); %#ok<NASGU>
...
fwrite(vr.fid,[rem(now,1) vr.position([1:2,4]) vr.velocity(1:2) vr.cuePos vr.isReward vr.inITI vr.greyFac vr.inBreak],'float');
