function code = GradedDelaySinRew
% GradedDelaySinRew   Code for the ViRMEn experiment GradedDelaySinRew.
%   code = GradedDelaySinRew   Returns handles to the functions that ViRMEn
...
if vr.inITI == 0 && abs(vr.position(1)) > eval(vr.exper.variables.armLength)/2 && vr.position(2) > eval(vr.exper.variables.MazeLengthAhead)
    ...
    if ((vr.cuePos == 1 || vr.cuePos == 2) && vr.isReward ~= 0) || ((vr.cuePos == 3 || vr.cuePos == 4) && vr.isReward == 0)
        vr.numBlackTurnsDelay = vr.numBlackTurnsDelay + 1;
...
if vr.inITI == 1
    ...
    if vr.itiTime > vr.itiDur
        ...
        if vr.numTrialsDelayLevel >= vr.trialThresh && vr.percGraded < 60
            vr.percCorrectLast = sum(vr.trialResults(1,(end-vr.trialThresh+1):end))/vr.trialThresh;
            if vr.percCorrectLast >= vr.percThreshAdv 
                if vr.percGraded > 0 && vr.percGraded < 35
                    vr.percGraded = vr.percGraded + 10;
                else
...
%         vr.percBlackDelay = vr.numBlackTurnsDelay/vr.numTrialsDelayLevel;
