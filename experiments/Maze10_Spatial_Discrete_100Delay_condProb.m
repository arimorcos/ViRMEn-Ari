function code = Maze10_Spatial_Discrete_100Delay_condProb
% Maze10_Spatial_Discrete_100Delay_condProb   Code for the ViRMEn experiment Maze10_Spatial_Discrete_100Delay_condProb.
%   code = Maze10_Spatial_Discrete_100Delay_condProb   Returns handles to the functions that ViRMEn
...
%discrete change control
vr.changeProb = false; %should activate changing probabilties
vr.condWin = 20; %number of trials to look at for window
vr.condThresh = 0.75; %percentage necessary to advance probability change
vr.condTimeOut = 3; %minimum number of trials between probability changes
vr.deltaProb = 0.03; %rate at which conditional probabilities change
vr.maxProbs = [1 0.35 0.15 0.1]; %maximum probability for each of the conditions
...
if vr.inITI == 1
    ...
    if vr.itiTime > vr.itiDur
        ...
        perf = zeros(1,(vr.numSeg/2)+1); %initialize performance array with as many conditions as are present
        if size(vr.trialResults,2) >= vr.condWin %if more trials than window to analyze 
            for i=1:(vr.numSeg/2)+1
                trialSub{i} = vr.trialResults(:,vr.trialResults(4,(end-vr.condWin+1):end)== i-1 |...
                    vr.trialResults(4,(end-vr.condWin+1):end)== vr.numSeg+1-i);
