function code = GradedBlockDelay2TowersContCrutch_630
% GradedBlockDelay2TowersContCrutch_630   Code for the ViRMEn experiment GradedBlockDelay2TowersContCrutch_630.
%   code = GradedBlockDelay2TowersContCrutch_630   Returns handles to the functions that ViRMEn
...
%blocks
vr.blockThresh = [0 5/63 10/63 15/63]; %thresholds for blocks
vr.trialBlockThresh = 5;
vr.percBlockThresh = 0.75;
vr.roundVec = 0:(1/63):1;

%delay advance
vr.advCount = 2; %net count value to advance
vr.regCount = -2; %net count value to regress
vr.advRate = 1/63; %must be related to number of textures
vr.regRate = 0/63;
vr.maxGrey = 15/63;
...
vr.blackLeftCrutch = [beginBlack vr.whiteRight backBlack vr.blackLeftTower(1):vr.blackLeftTower(2) vr.whiteRightTower(1):vr.whiteRightTower(2)];
vr.blackRightCrutch = [beginBlack vr.whiteLeft backBlack vr.blackRightTower(1):vr.blackRightTower(2) vr.whiteLeftTower(1):vr.whiteLeftTower(2)];
vr.whiteLeftCrutch = [beginWhite vr.whiteLeft backWhite vr.whiteLeftTower(1):vr.whiteLeftTower(2) vr.blackRightTower(1):vr.blackRightTower(2)];
vr.whiteRightCrutch = [beginWhite vr.whiteRight backWhite vr.whiteRightTower(1):vr.whiteRightTower(2) vr.blackLeftTower(1):vr.blackLeftTower(2)];
...
if vr.inITI == 1
    ...
    if vr.cellWrite
        [dataStruct] = createSaveStruct(vr.mouseNum,vr.experimenter,...
            ...
            'numRewPer',vr.numRewPer,'crutchTrial',vr.crutchTrial); %#ok<NASGU>
    ...
    if toc(vr.trialsStart) < vr.breakThreshTime || ~vr.breaks
        ...
        if vr.itiTime > vr.itiDur
            ...
            if sum(nonCrutchTrials(4,:)==vr.block)>=vr.trialBlockThresh
                trialResultsSeg = nonCrutchTrials(:,end+1-vr.trialBlockThresh:end);
                vr.percCorrectBlock = sum(trialResultsSeg(1,trialResultsSeg(4,:) == vr.block))/vr.trialBlockThresh;
            else
                vr.percCorrectBlock = sum(nonCrutchTrials(1,nonCrutchTrials(4,:)...
                    ==vr.block))/sum(nonCrutchTrials(4,:)==vr.block);
            end
